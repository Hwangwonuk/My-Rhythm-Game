...
public class MyRhythmGame extends JFrame {
	...
	private Music introMusic = new Music("introMusic.mp3", true); // 시작화면에서 음악이 직접 종료시키기 전까지 무한정 반복해서 재생이 이루어지도록 해준다.
	private int nowSelected = 0; // 그곡의 번호를 의미한다 index니까 0부터

	public static Game game; // 선언만하게 만듬
	
	public MyRhythmGame() {
		
		trackList.add(new Track("Inuyasha Title Image.png", "inuyasha Start Image.png", "Inuyasha Game Image.png",
				"Affection Touching Selected.mp3", "Affection_Touching_Across_Time.mp3", "Affection_Touching_Across_Time"));
		// 총 6개의 매개변수가 들어가는데 배열에 차례대로 들어간다. track 생성자를 확인. 0번째 인덱스가 된다.
		trackList.add(new Track("Inu Title Image.png", "Inu Start Image.png", "Inu Game Image.png",
				"Always with me Selected.mp3", "always_with_me_ocarina_ver.mp3", "always_with_me_ocarina_ver"));
		trackList.add(new Track("Howls Title Image.png", "Howls Start Image.png", "Howls Game Image.png",
				"Howls Moving Selected.mp3", "Howls_Moving_Castle.mp3", "Howls_Moving_Castle"));
		...
		backButton.setVisible(false); // 맨 처음엔 보이지 않다가 시작하기 버튼을 누르고 메인화면가면 메인 스크린에 나온다.
		backButton.setBounds(20, 50, 60, 60); // 메뉴바의 가장 오른쪽 부분에 위치하게된다.
		...
		backButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				backButton.setIcon(backButtonEnteredImage); // 마우스가 올라갔을 때 entered로 바꿔준다
				backButton.setCursor(new Cursor(Cursor.HAND_CURSOR)); // 손가락 모양으로 바뀐다
				Music buttonEnteredMusic = new Music("buttonEnteredMusic.mp3", false); // 한번만 음악실행
				buttonEnteredMusic.start();
			}
			...
			@Override
			public void mouseExited(MouseEvent e) {
				backButton.setIcon(backButtonBasicImage); // 마우스가 빠져나왔을 때 이미지
				backButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); // 원래 상태로 돌아온다
			}
	...
	public void paint(Graphics g) { // paint는 하나의 약속된 메소드 JFrame을 상속받은 GUI 게임에서 가장 첫번째로 화면을 그리는 함수
		...
		screenDraw((Graphics2D) screenGraphic); // screenDraw를 이용해서 screenGraphic에 그림을 그려준다. 아직 만들지 않음
												// Graphics2D를 import하고 형변환
		g.drawImage(screenImage, 0, 0, null); // screenImage를 0, 0의 위치에 그린다.

	}

	public void screenDraw(Graphics2D g) { // 2D로 적용
	...
	public void selectTrack(int nowSelected) {
		...
		titleImage = new ImageIcon(Main.class.getResource("..//images//" + trackList.get(nowSelected).getTitleImage()))
				.getImage(); // 현재 선택된 곡의 타이틀 이미지 값을 가져와서 넣는다
		selectedImage = new ImageIcon(
				Main.class.getResource("..//images//" + trackList.get(nowSelected).getStartImage())).getImage(); // 현재 선택된 이미지를 곡 이미지로 
																												 // 바꿔주고 제목 이미지또한 선택된 것으로 변경
		selectedMusic = new Music(trackList.get(nowSelected).getStartMusic(), true); // track list에서 nowselected를 가져와서
																					 // 음악도 가져온다 true값으로 무한재생
	...
	public void gameStart(int nowSelected, String difficulty) {
		...
		isGameScreen = true; // 게임 시작하면 isGameScreen 이미지 보이게
		game = new Game(trackList.get(nowSelected).getTitleName(), difficulty, trackList.get(nowSelected).getGameMusic());
		game.start(); // 해당 game 인스턴스의 run()이 자동으로 실행된다 Game class의 run메소드
		setFocusable(true); // gameStart화면에 Focus를 설정 키보드가 인식될수 있게
	}
	...
	public void backMain() {
		...
		selectTrack(nowSelected); // 다시 현재 선택된 트랙을 보여주고 하이라이트 부분을 재생시켜라
		isGameScreen = false;
		game.close();
	}
	...
	public void enterMain() {
		...
		background = new ImageIcon(Main.class.getResource("..//images//mainBackground.jpg")).getImage();
		isMainScreen = true; // 시작하기 버튼을 눌러서 메인화면으로 이동하는 부분
		leftButton.setVisible(true);
		rightButton.setVisible(true);
		easyButton.setVisible(true);
		hardButton.setVisible(true);
		introMusic.close(); // introMusic이 클래스 내부함수에서 선언이 되어서 오류발생
		selectTrack(0);
	}
}
